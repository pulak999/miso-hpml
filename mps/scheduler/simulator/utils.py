...
# Add repo root to path if not already there (for subdirectory imports)
...
def get_speedup(job_models, error_mean, error_std):
    ...
    perf_pred = {}
    mig_path = get_path('mps', 'models', 'logs', 'mig')
...
def implement_mapped_config(config_dict, gpu_list):
    ...
    with open(get_path('mps', 'scheduler', f'mapped_{num_gpu}gpu.json')) as f:
        mapping = json.load(f)
